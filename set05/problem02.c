...
void input_details(float *radius,float *height,float *length){
  printf("enter details(radius,height,length) of the camel");
    scanf("%f %f %f",radius,height,length);
}
float find_weight(float radius,float height,float length){
float x;
  x =  3.14 * radius* sqrt(height * length);
  return x;
}
void output(float radius,float length,float height,float weight){
  printf("weight of camel having radius: %.1f length: %.1f and weight: %.1f is %.2f",radius,length,height,weight);
}
int main()
{
